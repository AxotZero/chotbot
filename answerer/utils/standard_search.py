...
# PUBLIC
def search(query, start_num=0, total_num=5, lang='en', area='com', ncr=False, void=True, time_period=False, sort_by_date=False):
    ...
    url = _get_search_url(query, start_num=start_num, total_num=total_num, lang=lang, area=area, ncr=ncr, time_period=time_period, sort_by_date=sort_by_date)
    html = get_html(url)
...
# by axot
def get_wiki_description(query, lang='en', area='com'):
    answer = ''
    url = _get_search_url(query, start_num=0, total_num=1, lang=lang, area=area)
    html = get_html(url)
    if html:
